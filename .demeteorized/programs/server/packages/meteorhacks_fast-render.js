...
FastRender.route = function route(path, callback) {                                                               // 14
  fastRenderRoutes.route(path, FastRender.handleRoute.bind(null, callback));                                      // 15
};                                                                                                                // 16
                                                                                                                  // 17
function setQueryDataCallback(res, next) {                                                                        // 18
  return function(queryData) {                                                                                    // 19
    if(!queryData) return next();                                                                                 // 20
                                                                                                                  // 21
    var existingPayload = res.getData("fast-render-data");                                                        // 22
    if(!existingPayload) {                                                                                        // 23
      res.pushData("fast-render-data", queryData);                                                                // 24
    } else {                                                                                                      // 25
      // it's possible to execute this callback twice                                                             // 26
      // the we need to merge exisitng data with the new one                                                      // 27
      _.extend(existingPayload.subscriptions, queryData.subscriptions);                                           // 28
      _.each(queryData.collectionData, function(data, pubName) {                                                  // 29
        var existingData = existingPayload.collectionData[pubName]                                                // 30
        if(existingData) {                                                                                        // 31
          data = existingData.concat(data);                                                                       // 32
        }                                                                                                         // 33
                                                                                                                  // 34
        existingPayload.collectionData[pubName] = data;                                                           // 35
        res.pushData('fast-render-data', existingPayload);                                                        // 36
      });                                                                                                         // 37
    }                                                                                                             // 38
    next();                                                                                                       // 39
  };                                                                                                              // 40
                                                                                                                  ...
                                                                                                                  // 42
FastRender.handleRoute = function(processingCallback, params, req, res, next) {                                   // 43
  var afterProcessed = setQueryDataCallback(res, next);                                                           // 44
  FastRender._processRoutes(params, req, processingCallback, afterProcessed);                                     // 45
};                                                                                                                // 46
                                                                                                                  // 47
FastRender.handleOnAllRoutes = function(req, res, next) {                                                         // 48
  var afterProcessed = setQueryDataCallback(res, next);                                                           // 49
  FastRender._processAllRoutes(req, afterProcessed);                                                              // 50
};                                                                                                                // 51
                                                                                                                  // 52
FastRender.onAllRoutes = function onAllRoutes(callback) {                                                         // 53
  FastRender._onAllRoutes.push(callback);                                                                         // 54
};                                                                                                                // 55
                                                                                                                  // 56
FastRender._processRoutes =                                                                                       // 57
  function _processRoutes(params, req, routeCallback, callback) {                                                 // 58
  callback = callback || function() {};                                                                           // 59
                                                                                                                  // 60
  var path = req.url;                                                                                             // 61
  var loginToken = req.cookies['meteor_login_token'];                                                             // 62
  var headers = req.headers;                                                                                      // 63
                                                                                                                  // 64
  var context = new Context(loginToken, { headers: headers });                                                    // 65
                                                                                                                  // 66
  try {                                                                                                           // 67
    FastRender.frContext.withValue(context, function() {                                                          // 68
      routeCallback.call(context, params, path);                                                                  // 69
    });                                                                                                           // 70
    callback(context.getData());                                                                                  // 71
  } catch(err) {                                                                                                  // 72
    handleError(err, path, callback);                                                                             // 73
  }                                                                                                               // 74
};                                                                                                                // 75
                                                                                                                  // 76
FastRender._processAllRoutes =                                                                                    // 77
  function _processAllRoutes(req, callback) {                                                                     // 78
  callback = callback || function() {};                                                                           // 79
                                                                                                                  // 80
  var path = req.url;                                                                                             // 81
  var loginToken = req.cookies['meteor_login_token'];                                                             // 82
  var headers = req.headers;                                                                                      // 83
                                                                                                                  // 84
  new Fiber(function() {                                                                                          // 85
    var context = new Context(loginToken, { headers: headers });                                                  // 86
                                                                                                                  // 87
    try {                                                                                                         // 88
      FastRender._onAllRoutes.forEach(function(callback) {                                                        // 89
        callback.call(context, req.url);                                                                          // 90
      });                                                                                                         // 91
                                                                                                                  // 92
      callback(context.getData());                                                                                // 93
    } catch(err) {                                                                                                // 94
      handleError(err, path, callback);                                                                           // 95
    }                                                                                                             // 96
  }).run();                                                                                                       // 97
};                                                                                                                // 98
                                                                                                                  // 99
function handleError(err, path, callback) {                                                                       // 100
  var message =                                                                                                   // 101
    'error on fast-rendering path: ' +                                                                            // 102
    path +                                                                                                        // 103
    " ; error: " + err.stack;                                                                                     // 104
  console.error(message);                                                                                         // 105
  callback(null);                                                                                                 // 106
}                                                                                                                 // 107
                                                                                                                  // 108
// adding support for null publications                                                                           // 109
FastRender.onAllRoutes(function() {                                                                               // 110
  var context = this;                                                                                             // 111
  var nullHandlers = Meteor.default_server.universal_publish_handlers;                                            // 112
                                                                                                                  // 113
  if(nullHandlers) {                                                                                              // 114
    nullHandlers.forEach(function(publishHandler) {                                                               // 115
      // console.log(publishHandler.toString());                                                                  // 116
      var publishContext = new PublishContext(context, null);                                                     // 117
      var params = [];                                                                                            // 118
      context.processPublication(publishHandler, publishContext, params);                                         // 119
                                                                                                                  // 120
    });                                                                                                           // 121
  }                                                                                                               // 122
});                                                                                                               // 123
